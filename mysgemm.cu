...
__global__ void mysgemm_cache_AB_prefetching_double_buffering(const float alpha, const float * __restrict__ dA, const int lda, const float * __restrict__ dB, const int ldb, const float beta, float * __restrict__ dC, const int ldc)
{
	...
	__shared__ float A_smem[BM][2 * BK + 1];
	__shared__ float B_smem[2 * BK][BN + 1];
...
//	float reg_b[BK / BY][BN / BX];
	...
	for(kk = 0; kk < lda; kk += BK) {
		...
		for(m = 0; m < BM; m += AY) 
			#pragma unroll
			for(n = 0; n < BK; n += AX) {
				A_smem[idyA + m][idxA + n + STORE] = daptr[m * lda + n];
		}
		...
		for(m = 0; m < BK; m += BY)
			#pragma unroll
			for(n = 0; n < BN; n += BX) {
				B_smem[idyB + m + STORE][idxB + n] = dbptr[m * ldb + n];
		}
		}
		...
		for(k = 0; k < BK; k++) {
			...
			#pragma unroll
			for(m = 0; m < BM / TY; m++) {
				reg_A[m] = A_smem[m * TY + idy][k + LOAD];
			}

			#pragma unroll
			for(n = 0; n < BN / TX; n++) {
				reg_B[n] = B_smem[k + LOAD][n * TX + idx];
			}	
